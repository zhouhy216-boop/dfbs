...
# 6) 讨论补充（2026-02-11）——以本节为准（覆盖前文 Unknown 口径）
## 6.1 轮子决策与试点顺序（已冻结）
- 轮子：是（长期通用）。
- 试点：先做 **工单 + 平台申请**（四个入口：服务/销售/替企/新建机构）。
- 说明：平台申请四类你已按“总表单 + 节点差异”逻辑做完，但现状不是按轮子实现；需要一次“事实对齐”后再迁入轮子。

## 6.2 权限与模板发布（已冻结）
- 模板配置者：仅 admin。
- 无权限体验：隐藏（入口/按钮/字段）。
- 模板生效：立即生效（不做定时生效）。
- 条件复杂度护栏：最多 2 层组合（外层 AND/OR；内层为条件列表/组）。

## 6.3 模板选择开关（已冻结）
- 工单：按 **工单类型** 选择模板；类型枚举：维修 / 投诉 / 培训 / 改造。
- 平台申请：按 **申请类型** 选择模板（服务/销售/替企/新建机构）；若未来出现二级类型再扩展。
...
## 6.5 工单流程节点（已冻结）
- 工单节点顺序（人话口径）：
  客户报修 → 客服受理 → 客服派单 → 工程师接单 → 工程师处理 → 客服回访 → 关闭

## 6.6 明细表归属与边界（关键：不跑到“智能粘贴解析”）
- 明细表（表格行）属于“表单自定义轮子”的字段类型之一。
- 本轮子内新增一个子能力块：**明细表批量录入（粘贴映射/训练 + 连号/自动填充）**。
- 严格边界：只处理“表格/列结构”的粘贴与生成；不做自由文本的智能解析（自由文本解析在《智能粘贴解析》THREAD 里讨论）。

## 6.7 明细表批量录入增强（新增需求，归属：表单轮子子能力）
### 6.7.1 典型业务场景
- 需要“基本信息表 + 明细表”：
  - 基本信息：客户名/地址/型号&台数等共性信息
  - 明细表：多台机器/多张卡的编号列表（如一次发 20 台、两型号各 10 台；开卡 10 张通常连号）

### 6.7.2 必须支持（已明确需求）
1) Excel 批量粘贴
- 支持桌面 Excel 复制粘贴；
- 支持网页上预览的 Excel 表格直接复制粘贴；
- 高频场景：粘贴一列（亦需兼容多列）。

2) 粘贴清洗
- 去空行、去首尾空格、去不可见字符/全角空格（网页复制常见）。

3) 自动填充 / 连号生成
- 你输入第一条（例如 LA26A207001），根据“数量=10”自动生成：
  LA26A207002 … LA26A207010
- 既支持字母数字混合编号，也支持纯数字编号（开卡常见）。
- 支持按“基本信息里的型号&台数”分组生成：两型号各 10 台时，明细表自动分两段填充，并自动填上对应型号字段。

4) 机器编号形态
- 常见：字母数字混合（示例：LA25A206006）
- 个别：纯数字

### 6.7.3 粘贴映射/训练（仍归属明细表批量录入，不升级为智能解析）
- 训练保存范围：按模板（你先配模板，再按模板训练）。
- 来源识别：有表头用表头；无表头用列数 + 内容特征。
- 同入口多模板（发货/委托共用粘贴入口）：
  - 允许直接粘贴；系统自动判模板；失败时再让用户选择模板/去训练。
  - 多模板同时命中：自动选命中度最高，并提示可切换。
- 交互：这是“省事功能”，不强制每次预览确认；解析/生成后直接写入明细表，用户可再修改后提交。
- 校验：第一版只做“本次明细表内不重复”（按你口径 A）。

---

# 7) 更新后的 Unknown（以本节为准）
1) 平台申请：
- 需要一次“事实对齐”：节点/字段矩阵/按钮矩阵/条件表达式的权威来源（现有实现或规则文档），然后迁入轮子。

2) 发货/委托/开卡（物联网卡申请）：
- 流程节点列表（谁在哪个节点填什么）
- 基本信息字段清单 + 明细表字段清单（至少“机器/卡编号、型号/套餐、备注”等）

3) 明细表引用选择（行内引用）
- 首批需要引用选择哪些对象（客户/产品/设备/SIM/仓库物料等）
- 每次引用后需要回填哪些字段（不提供也可先做“文本录入+粘贴+连号”）

4) 连号生成细则（可后补，不阻塞 v1）
- 递增段是否永远为末尾数字、是否要严格保留前导 0、是否存在步长/跳号规则等

---

# 8) 下次继续需要的实物（更新）
- 工单：各节点“字段/按钮矩阵”最小版（先挑 1-2 个节点也行）。
- 平台申请：现有实现的事实对齐材料（截图/规则文档/代码事实皆可）。
- 发货/委托/开卡：基本信息字段清单、明细表字段清单、两型号各 10 台这类样例（起始号/数量/型号分配）。
