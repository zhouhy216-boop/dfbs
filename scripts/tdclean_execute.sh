...
# Args: --moduleIds <comma-separated>, --confirmText <string|empty> (optional)
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --confirmText) CONFIRM_TEXT="$2"; shift 2 ;;
    *) shift ;;
  esac
done
...
if [ -n "$CONFIRM_TEXT" ]; then
  BODY=$(node -e "console.log(JSON.stringify({moduleIds:JSON.parse(process.argv[1]),confirmText:process.argv[2],includeAttachments:false}))" "$MODULES_JSON" "$CONFIRM_TEXT")
else
  BODY=$(node -e "console.log(JSON.stringify({moduleIds:JSON.parse(process.argv[1]),confirmText:null,includeAttachments:false}))" "$MODULES_JSON")
fi

TMP=$(mktemp)
trap 'rm -f "$TMP"' EXIT
HTTP_CODE=$(curl -s -w "%{http_code}" -o "$TMP" -X POST "${BASE}/api/v1/admin/test-data-cleaner/execute" \
...
if [ "$HTTP_CODE" = "200" ]; then
  STATUS=$(echo "$RESP_BODY" | node -e "try{var d=JSON.parse(require('fs').readFileSync(0,'utf8')); process.stdout.write(d.status||'');}catch(e){process.stdout.write('');}")
  case "$STATUS" in
    SUCCESS) exit 0 ;;
    PARTIAL) exit 2 ;;
    *)       exit 1 ;;
