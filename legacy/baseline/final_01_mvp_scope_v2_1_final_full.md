...
> 本文件仅定义业务与系统边界；
> 所有协作方式、输出规范、文件修改规则，
> 以 PROJECT_STATUS.md 中「协作与输出约定（冻结）」为唯一约束来源。

---

## 0. 冻结要点导航（只做索引，不替代正文）

> 说明：你要求“详细且不丢失”，所以正文保持完整；本导航只用于快速定位关键规则（尤其是高风险财务/边界点），避免遗漏导致返工。

### 0.1 报价自动建议（Price Suggestion）停止建议死线（高风险）
- 报价单一旦**首次递交**：系统必须**永久停止价格自动建议**；此后只能人工修改（并记录审计）。
- 目的：防止合同/价格条款变更导致历史报价“跳价”。

### 0.2 回款核销（Payment Allocation）跨周期分配（高风险）
- 允许跨周期：Payment 与 Allocation 可属于不同会计周期（Period）。
- 数据结构必须包含：allocated_amount、period_id（以及 allocated_at/allocated_by 等审计字段）。

### 0.3 仓库模型（Inventory）两级结构 + 出库绑定报价明细（高风险）
- 两级仓库：服务部大库 ↔ 服务经理小库（补货/领用/归还需留痕）。
- 出库必须绑定报价明细（明细级出库），避免无依据出库。

### 0.4 收入 vs 成本（双口径分离，高风险）
- 同一物流事件需同时记录“报价收入”和“报销成本(Expense)”两套口径；两者必须分离。

### 0.5 货损（Damage）独立性边界（高风险）
- 货损台账与维修/收费完全独立，不自动联动（避免业务耦合）。
...
#### 2.1.1 合同与价格条款（V1 范围明确）

- 合同模块的核心职责：
    - 合同号全局唯一
    - 合同作为“合同档案”，用于客户合同关系管理与审计追溯
    - 合同可关联附件（扫描件等）

- 合同不解析合同文本内容，不从合同文本中自动提取价格条款。

- 为支持报价模块自动引用价格，V1 引入“合同价格条款（ContractPricing）”结构化数据：
    - 每个合同 **只对应一套价格规则**
    - 一个客户可有多个合同，因此可存在多套价格规则
...
- 合同价格条款支持：
    - 生效起止日期（用于新合同自动覆盖旧合同）
    - 优先级（用于特殊协议优先）
    - 报价时按规则自动选择（如最低价 / 最新生效）

- 合同价格条款可关联对应合同档案作为依据，但不影响合同档案的独立性。
